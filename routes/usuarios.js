...
/**
 * Listar médicos disponibles (acceso autenticado)
 */
router.get("/medicos", verifyToken, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Médicos obtenidos exitosamente", data: medicos });
  } catch (error) {
...
router.put("/perfil", verifyToken, async (req, res) => {
  try {
    ...
    await pool.execute(
      "UPDATE usuarios SET nombre = COALESCE(?, nombre), telefono = COALESCE(?, telefono), especialidad = COALESCE(?, especialidad) WHERE id = ?",
      [nombre || null, telefono || null, especialidad || null, req.user.id]
    );
